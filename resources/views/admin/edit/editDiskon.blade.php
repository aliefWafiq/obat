...
<div class="main-content">
    <header class="header">
        <div class="header-left">
            ...
            <div>
                <h1 id="page-title">Edit Diskon</h1>
            </div>
        </div>
    </header>
    ...
    <section class="content-section active" style="display: flex; justify-content: center; padding: 2rem 1rem;">
        <div style="width: min(720px, 100%); background: #ffffff; border-radius: 24px; padding: 2rem; box-shadow: 0 18px 40px rgba(0,0,0,0.08);">
            ...
            <form action="{{ route('updateDiskon', $buatDiskon->id) }}" method="POST" enctype="multipart/form-data" class="admin-form">
                @csrf
                @method('PUT')
                ...
                <select name="produk_id" id="produk_id" style="width: 100%; padding: 0.85rem 1rem; border-radius: 12px; border: 1px solid #d1d5db; margin-bottom: 1.2rem; font-family: inherit;">
                    ...
                    @foreach ($produk as $p)
                        <option value="{{ $p->id }}" {{ $buatDiskon->idProduk == $p->id ? 'selected' : '' }}>{{ $p->namaProduk }}</option>
                    @endforeach
                </select>
                
                <p>Minimal Beli</p>
                <input type="number" name="minimalBeli" id="minimalBeli" placeholder="Contoh: 10" value="{{ $buatDiskon->minimalBeli }}" style="width: 100%; padding: 0.85rem 1rem; border-radius: 12px; border: 1px solid #d1d5db; margin-bottom: 1.2rem; font-family: inherit; box-sizing: border-box;">
                
                <p>Persentase Diskon (%)</p>
                <input type="number" name="diskon" id="diskon" placeholder="Contoh: 25" step="0.1" min="0" max="100" value="{{ $buatDiskon->diskon }}" style="width: 100%; padding: 0.85rem 1rem; border-radius: 12px; border: 1px solid #d1d5db; margin-bottom: 1.2rem; font-family: inherit; box-sizing: border-box;">
